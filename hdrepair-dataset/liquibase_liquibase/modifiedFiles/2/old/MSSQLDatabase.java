...
//    }
    ...
    public String getViewDefinition(CatalogAndSchema schema, String viewName) throws DatabaseException {
        ...
        finalDef = INITIAL_COMMENT_PATTERN.matcher(finalDef).replaceFirst("").trim(); //handleOperation views that start with '/****** Script for XYZ command from SSMS  ******/'
        finalDef = CREATE_VIEW_AS_PATTERN.matcher(finalDef).replaceFirst("").trim();

        finalDef = finalDef.replaceAll("--.*", "").trim();

        /**handleOperation views that end up as '(select XYZ FROM ABC);' */
